...
def solve_tau(v, w, mu, rho, l, u, tol=1e-12):
    def f(tau):
        return tau - rho * np.sum(np.abs(x_vec(v, w, tau, mu, l, u)))
    lo, hi = 0, rho*np.sum(np.maximum(np.abs(l), np.abs(u)))
    while f(hi) < 0: hi *= 2.0
    while hi - lo > tol:
        mid = (lo + hi) / 2
...
def prox_sqL1_box(v, rho, w, alpha, l, u,
                  tau_tol=1e-12, mu_tol=1e-12, max_iter=100, tolc = 1e-6):
    ...
    while True:
        ...
        mu_hi *= 2
    while mu_hi - mu_lo > mu_tol and np.linalg.norm(x-w,1) > alpha + tolc:
        mu   = (mu_lo + mu_hi) / 2
    ...
    return x, mu
def prox_sqL1_unconstrained(q, rho_t, tol=1e-12):
    """argmin_x ½‖x-q‖₂² + ½ρ_t‖x‖₁²"""
    if rho_t == 0:
        return q.copy()
...
def admm_sqL1_box(v, rho, w, alpha, l, u,
                  beta=1.0, max_iter=100000, tol=1e-6, log_every=100, thres=None):
    v, w, l, u = map(lambda a: np.asarray(a,float).ravel(), (v,w,l,u))
    x = np.clip(v, l, u)
    z = x.copy()
...
def run_one_trial(seed, d):
    ...
    F_ex = F(x_ex, v, rho)
    beta = 100+300*np.log2(d)  # beta for ADMM
    # --- ADMM
    t0 = time.perf_counter()
    x_adm = admm_sqL1_box(v, rho, w, alpha, l, u,
                          beta=beta, log_every=0, max_iter=max_it, thres=F_ex, tol=tol)
...
with open(csv_path, "w", newline="") as fh:
    ...
    for d in dims:
        for t in range(trials):
            res = run_one_trial(seed=10+t, d=d)
            wr.writerow([d, t, *res])
