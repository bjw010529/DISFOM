...
os.makedirs("./data/save_para", exist_ok=True)
...
def DIFOM_minibatch(tmpsigma, xop, x0, learning_rate, M, batch_size, t=0):
    ...
    while True:
        ...
        x = prox_squared_l1_box_colvec(x - learning_rate * grad, x, -R*np.ones_like(x), R*np.ones_like(x), 2, 1e-12)
        if epoch >= 50:
...
def DIFOM_svrg(tmpsigma, xop, x0, learning_rate, M, batch_size_1, t=0):
    ...
    while True:
        if cnt % epoch_length == 0:
            ...
            epoch += 1
            cnt += 1
            x2 = prox_squared_l1_box_colvec(x - learning_rate * nk_grad, x, -R*np.ones_like(x), R*np.ones_like(x), 10, epsi_hat)
            x = x2
            tmp_loss = qua_loss(Sigma, xop, x, lambda_reg)
            ...
            x2 = prox_squared_l1_box_colvec(x - learning_rate * grad_corr, x, -R*np.ones_like(x), R*np.ones_like(x), 10, epsi_hat)
            x = x2
            tmp_loss = qua_loss(Sigma, xop, x, lambda_reg)
            measure = qua_measure(Sigma, xop, x, lambda_reg)
...
np.savetxt(f"./data/save_para/cpu_time_{d}_difom.csv", cpu_difom)
np.savetxt(f"./data/save_para/cpu_time_{d}_prox.csv",  cpu_prox)
np.savetxt(f"./data/save_para/cpu_time_{d}_smd.csv",   cpu_smd)
np.savetxt(f"./data/save_para/cpu_time_{d}_svrg.csv",  cpu_svrg)
np.savetxt(f"./data/save_para/cpu_time_{d}_difom_svrg.csv", cpu_difom_svrg)
np.savetxt(f"./data/save_para/cpu_time_{d}_smd_svrg.csv",   cpu_smd_svrg)
